...
{
    public class _Astar_Test
    {
		#region Singleton
		private static _Astar_Test instance;
		public static _Astar_Test Instance
		{
			get
			{
				if (instance == null)
				{
					instance = new _Astar_Test();
				}
				return instance;
			}
		}
		...
		{
			if (!close.Contains(cell) && !open.Contains(cell) && cell.TileType == ETileType.Grass && cell.IsUnitOccupied == false)
			{
				AddOpen(cell, gCost);
			}
		}
